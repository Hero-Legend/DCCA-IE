...
if [[ "$CACHEDIR" = "" ]]; then
	...
	if [[ "$OSTYPE" = "darwin"* ]]; then
		...
		if [[ -e $CACHEDIR/tensoradapter_pytorch_cpu ]]; then
			...
			cp -v $CACHEDIR/dgl_sparse_cpu/*.so build/dgl_sparse
		fi
	else
		if [[ $USE_CUDA = 'ON' ]]; then
			...
			if [[ -e $CACHEDIR/tensoradapter_pytorch_cu$CUDA_VER ]]; then
				...
				cp -v $CACHEDIR/dgl_sparse_cu$CUDA_VER/*.so build/dgl_sparse
			fi
		else
			echo Copying prebuilt CPU binary...
...
rm -f ../build/libdgl.so ../build/dgl_sparse/*.so
